...
from ..cruds.cruds import get_login, signin, signup, get_all_events,\
    ...
    get_votetime, generate_votetime, vote_register, vote_delete, get_voteresult, get_friendcal
router = APIRouter()
...
async def get_friend_cal(fid: int, user: str = Depends(authenticate), db: Session = Depends(get_db)):
